        ...
        while (txvalid || rxvalid || (!txeof) || (!rxeof)) {
            ...
            while ((!txvalid) && (!txeof)) {
                ...
                if (std::getline(txstream, line)) {
                    txvalid = old_str_to_umi_packet(line, (uint32_t*)txp.data);
                    if (!txvalid) {
            ...
            while ((!rxvalid) && (!rxeof)) {
                ...
                if (std::getline(rxstream, line)) {
                    rxvalid = old_str_to_umi_packet(line, (uint32_t*)rxp.data);
                    if (!rxvalid) {
            ...
            if (rx.is_active()) {
                ...
                if (rx.recv(outp)) {
                    ...
                    if (outstream.is_open()) {
                        outstream << old_umi_packet_to_str((uint32_t*)outp.data) << std::endl;
                    }

                    // if there's an expected RX packet, check against it
                    if (rxvalid) {
                        if (!old_umi_packets_match((uint32_t*)outp.data, (uint32_t*)rxp.data)) {
